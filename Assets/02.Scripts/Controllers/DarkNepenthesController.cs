...
namespace Platformer.Controllers
{
    public class DarkNepenthesController : EnemyController
    {
        protected override void Awake()
        {
            ...
            {
               var projectile =  PoolManager.instance.Get<ProjectileWithDamage>(PoolTag.Projectile_DarkNepenthes);
                projectile.transform.position = transform.position + new Vector3(direction * 0.1f, 0.2f, 0.0f);
                projectile.owner = transform;
                projectile.velocity = Vector3.right * direction * 5.0f;
                projectile.damage = Random.RandomRange(damageMin, damageMax);
